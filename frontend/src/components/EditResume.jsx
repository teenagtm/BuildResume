...
import Input, { TitleInput, ProfilePhotoSelector } from "./Input";
...
// Converts a base64 data URL to a File object safely
...
//take back to previous form state or it will just take you /dashboard if you are on first page
  const goBack = () => {
    const pages = [
      "profile-info",
      "contact-info",
...
//fetching resume details by id or using backend url
...
//it will help in choosing the preview as well as helps in downloading the resume
...
//store the image as resume and style will be png
...
//this funtion will help in updating the resume and put will help in updation on backend 
...
//delete function to delete any resume
  ...
  //download function help us in downloading the pdf
  ...
  //type of how it will look when downloaded
    try {
      await html2pdf()
        ...
        .from(element)
        .save();//save the resume and download
...
//theme selector sunction
  ...
  //so all these function will be used for the editing of resume
    ...
    <DashboardLayout>
        <div className={containerStyles.main}>
            <div className={containerStyles.header}>
                ...
                <button  className={buttonStyles.theme} onClick={() => setOpenThemeSelector(true)}>
                    <Palette size={16}/>
            ...
            {/* Theme Selector Modal */}
               <Modal 
               show={openThemeSelector} 
               onHide={() => setOpenThemeSelector(false)} 
               centered
               >
  ...
  centered 
  size="lg"
  onEntered={downloadPDF}   // 🔥 Modal open होते ही PDF download trigger होगा
>
  <Modal.Header closeButton>
    <Modal.Title>{resumeData.title}</Modal.Title>
  </Modal.Header>
  ...
  <Modal.Footer>
    <button
      className="px-6 py-3 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white font-black rounded-2xl hover:scale-105 transition-all shadow-lg hover:shadow-xl flex items-center gap-2"
      onClick={downloadPDF}
    ...
    >
      ...
      {isDownloading ? "Downloading..." : downloadSuccess ? "Downloaded" : "Download PDF"}
               ...
               {/* <Modal isOpen={openPreviewModal} onClose={()=>setOpenPreviewModal(false)}
                ...
                showActionBtn
                actionBtnText={isDownloading ? "Downloading..." : downloadSuccess ? "Downloaded" : "Download PDF"}
                ...
                <div style={{ visibility: "hidden", position: "absolute", left: "-9999px", top: "0" }}
                 ref={thumbnailRef}>
                  <div className={containerStyles.hiddenThumnail}>
